...
// Type: PathMedical.PatientManagement.Command.CutTestCommand
...
public class CutTestCommand : CommandBase
{
  private readonly PatientManager model;
  private object audiologyTest;

  public CutTestCommand(PatientManager model)
  {
    if (model == null)
      throw ExceptionFactory.Instance.CreateException<ArgumentNullException>(nameof (model));
    this.Name = nameof (CutTestCommand);
    this.model = model;
  }

  public override void Execute() => this.model.TestToCopy = this.model.SelectedAudiologyTest;
}
