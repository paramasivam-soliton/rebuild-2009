// Decompiled with JetBrains decompiler
// Type: PathMedical.PatientManagement.PatientManagerException
// Assembly: PM.PatientManagement, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 8132C915-2847-4A62-9EDC-EAE2A0AF1229
// Assembly location: C:\Users\paramasivam.g\Downloads\Release\Release\Base\PM.PatientManagement.dll

using System;

#nullable disable
namespace PathMedical.PatientManagement;

public class PatientManagerException : Exception
{
  public PatientManagerException()
  {
  }

  public PatientManagerException(string message)
    : base(message)
  {
  }

  public PatientManagerException(string message, Exception inner)
    : base(message, inner)
  {
  }
}
